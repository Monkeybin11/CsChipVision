...
namespace LedChipPassFail_first.Func
{
    ...
    public static class ImgPFunc
    {
        ...
        public static Func<double, double, double[,,]> FnCreateEstedChipPos(double realImgROIH, double realImgROIW , double hoffset , double woffset )
        {
            var createEsted = fun( (double hChipN,double wChipN) => {
                ...
                for (int j = 0; j < hChipN; j++)
                {
                    ...
                    {
                        output[j,i,0] = realImgROIH / (hChipN-1) * j + hoffset;
                        output[j,i,1] = realImgROIW / (wChipN-1) * i + woffset;
                    }
                }
                return output; // [Y,X,V]
            } );
        ...
        public static Func<double , double , bool> FnInBox( Rectangle box,int margin ) {
            var inbox = fun((double y,double x)=> {
            ...
            var sumareap = fun((int y, int x)=>
            {
                ...
                try
                {
                    foreach (var item in index)
                    {
                        x1 = item[0];
                        y1 = item[1];
                        output += img.Data[ y1, x1, 0 ];
                        //Console.WriteLine($"{i} , {j} Done");
                    }
                }
                catch (Exception)
                {
